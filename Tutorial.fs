...
module PipelinesAndComposition =
    ...
    let squareOddValuesAndAddOneNested values = List.map addOne (List.map square (List.filter isOdd values))
    ...
    let squareOddValuesAndAddOneComposition = List.filter isOdd >> List.map (square >> addOne)
...
module RecordTypes =
    ...
    type ContactCardAlternate =
        ...
        member this.PrintedContactCard =
            this.Name + " Phone: " + this.Phone + (if not this.Verified then " (unverified)"
                                                   else "")
            + this.Address
...
module DefiningGenericClasses =
    ...
    type StateTracker<'T>(initialElement : 'T) =
        ...
        member this.UpdateState newState = states <- newState :: states // use the '<-' operator to mutate the value.
